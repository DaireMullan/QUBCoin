/**
 * This chaincode implements the ERC20 standard token for QUBCoin
...
func (token *ERC20TokenChaincode) Init(stub shim.ChaincodeStubInterface) peer.Response {
	...
	if len(args) < 4 {
		return errorResponse("Initialization failure - incorrect number of parameters", 3)
	}
	...
	if err != nil || totalSupply == 0 {
		return errorResponse("Total Supply must be a positive integer", 3)
	}

	// Creator name cannot be zeo length
	if len(args[3]) == 0 {
		return errorResponse("Creator identity must be supplied to initialize chaincode", 3)
	}
...
func balanceOf(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	if len(args) < 1 {
		return errorResponse("No OwnerID supplied", 6)
	}
	OwnerID := args[0]
...
func transfer(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	// Check if owner id is in the arguments
	if len(args) < 3 {
		return errorResponse("QUBCoin transfer needs to, from & amount variables", 700)
	}
	...
	if amount <= 0 {
		return errorResponse("Transfer amount MUST be > 0", 702)
	}
	...
	if len(bytes) == 0 {
		...
		return errorResponse("User account balance MUST be > 0", 703)
	}
...
func successResponse(successMessage string) peer.Response {
	success := "{\"response\": " + successMessage + ", \"code\": 0 }"
	return shim.Success([]byte(success))
}

// Chaincode registers with the Shim on startup
func main() {
	fmt.Println("Starting QUBCoin ERC20 token chaincode")
